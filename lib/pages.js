...
function renderCouncilBody(council, { heading, lede, extraNote }) {
  const widgetOrLink = council.supported
    ...
    : `
      <div class="official-link-card">
        <div>
          <p class="official-link-title">We haven't built a live lookup for ${council.name} yet</p>
          <p class="official-link-detail">${council.name} runs their own bin-day checker directly — it'll have your real, accurate collection dates.</p>
        </div>
  ...
  const finalLede = lede || `${council.name} isn't on our live lookup yet — but here's where to find your real bin day directly.`;
